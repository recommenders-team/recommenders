...
class SUMModel(SequentialBaseModel): 
    ...
    def _build_seq_graph(self):
        ...
        with tf.variable_scope("sum"):      
            ...
            final_state, att_weights = self._attention_query_by_state(final_state, self.target_item_embedding) 
    ...
    def _attention_query_by_state(self, seq_output, query):  
        ...
        with tf.variable_scope("query_att"):  
            if self.hparams.slots > 1 :
                ...
                memory_state = tf.reshape(seq_output, [-1, self.hparams.slots, self.hidden_size])  
                ...
                att_res = tf.reduce_sum(memory_state * tf.expand_dims(att_weights, -1), 1)   
    ...
    def _create_sumcell(self):
        ...
        input_params = [hparams.hidden_size * hparams.slots + input_embedding_dim, hparams.slots, hparams.attention_size, input_embedding_dim]  
        sumcells = {
            "SUM": SUMCell,            
            "SUMV2": SUMV2Cell
        }
    ...
    def _build_sum(self, cell):
        ...
        with tf.variable_scope("sum"): 
            ...
            rum_outputs, final_state = dynamic_rnn(
                ...
                scope="sum",
                initial_state=cell.zero_state(tf.shape(self.history_embedding)[0], tf.float32)
            )
