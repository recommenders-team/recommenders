...
class SUMCell(LayerRNNCell):
    ...
    def _basic_build(self, inputs_shape):
        ...
        self._erase_b = self.add_variable(
            name="_erase_b", shape=[h], initializer=(
        ...
        self._reset_b = self.add_variable(
            name="_reset_b", shape=[1], initializer=(
                self._bias_initializer
                if self._bias_initializer is not None
                else init_ops.constant_initializer(1.0, dtype=self.dtype)
            )
        ...
        self._add_b = self.add_variable(
            name="_add_b", shape=[h], initializer=(
                self._bias_initializer
                if self._bias_initializer is not None
                else init_ops.constant_initializer(1.0, dtype=self.dtype)
            )
    ...
    def build(self, inputs_shape):
        ...
        self.parameter_set = [
            self._erase_W, self._erase_b, self._reset_W, self._reset_b, self._add_W, self._add_b, self.heads
        ]
    ...
    def call(self, inputs, state): 
        ...
        reset = tf.sigmoid(tf.nn.xw_plus_b(tf.concat([inputs, h_hat], axis=-1), self._reset_W, self._reset_b))   
        erase = tf.sigmoid(tf.nn.xw_plus_b(tf.concat([inputs, h_hat], axis=-1), self._erase_W, self._erase_b))   
        add = tf.tanh(tf.nn.xw_plus_b(tf.concat([inputs, reset * h_hat], axis=-1), self._add_W, self._add_b))  
...
class SUMV2Cell(SUMCell):
    ...
    def build(self, inputs_shape):
        ...
        self._writing_b = self.add_variable(
            name="_writing_b", shape=[h], initializer=(
                self._bias_initializer
                if self._bias_initializer is not None
                else init_ops.constant_initializer(1.0, dtype=self.dtype)
            )
        ...
        self.parameter_set = [
            self._erase_W, self._erase_b, self._reset_W, self._reset_b, self._add_W, self._add_b, self.heads, self._writing_W, self._writing_W02, self._writing_b
        ]
