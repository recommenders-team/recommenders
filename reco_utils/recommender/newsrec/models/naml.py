...
class NAMLModel(BaseModel):
    ...
    def _build_userencoder(self, newsencoder):
        ...
        his_input_title_body_verts = keras.Input(shape=(hparams.his_size, 
                                                hparams.title_size+hparams.body_size+2),dtype='int32')
    ...
    def _build_vertencoder(self):
        ...
        vert_embedding = layers.Embedding(
                        hparams.vert_num,
                        hparams.vert_emb_dim,
                        trainable=True)

        vert_emb = vert_embedding(input_vert)
        pred_vert = layers.Dense(
            hparams.filter_num, 
            activation=hparams.dense_activation)(vert_emb)
    ...
    def _build_subvertencoder(self):
        ...
        pred_subvert = layers.Dense(
            hparams.filter_num, 
            activation=hparams.dense_activation)(subvert_emb)
    ...
    def _build_naml(self):
        ...
        pred_title_body_verts_one = layers.Concatenate(axis=-1)(
            [pred_input_title_one, pred_input_body_one, pred_input_vert_one, pred_input_subvert_one]
        )
        ...
        model = keras.Model(
            [imp_indexes, user_indexes, his_input_title, 
            his_input_body, his_input_vert, his_input_subvert,
            pred_input_title, pred_input_body, pred_input_vert, pred_input_subvert], preds)

        scorer = keras.Model([
            imp_indexes, user_indexes, his_input_title, 
            his_input_body, his_input_vert, his_input_subvert,
            pred_input_title_one, pred_input_body_one, pred_input_vert_one,
