...
class NPAModel(BaseModel):
    ...
    def _build_newsencoder(self, embedding_layer, user_embedding_layer):
        ...
        '''
        hparams = self.hparams
        sequence_title_uindex = keras.Input(shape=(hparams.doc_size+1,),dtype='int32')

        sequences_input_title = layers.Lambda(lambda x: x[:, :hparams.doc_size])(sequence_title_uindex)
        user_index = layers.Lambda(lambda x: x[:, hparams.doc_size:])(sequence_title_uindex)
        ...
        pred_title = PersonalizedAttentivePooling(hparams.doc_size, hparams.filter_num, hparams.attention_hidden_dim)(
    ...
    def _build_npa(self):
        ...
        pred_title_uindex_one = layers.Concatenate()([pred_input_title_one, user_indexes])
