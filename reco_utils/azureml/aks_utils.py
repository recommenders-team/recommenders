...
def qps_to_replicas(target_qps, processing_time, max_qp_replica=1, target_utilization=0.7, verbose=True):
    ...
    if verbose:
        print('Approximately {} replicas are estimated to support {} queries per second.'.format(replicas, target_qps))
    return replicas
...
def replicas_to_qps(num_replicas, processing_time, max_qp_replica=1, target_utilization=0.7, verbose=True):
    ...
    if verbose:
        print('Approximately {} queries per second are supported by {} replicas.'.format(qps, num_replicas))
    return qps
...
def total_cores_to_replicas(n_cores, cpu_cores_per_replica=0.1, overhead=0.1, verbose=True):
    ...
    if verbose:
        print('Approximately {} replicas are supported by {} cores.'.format(replicas, n_cores))
    return replicas