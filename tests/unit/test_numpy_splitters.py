...
from reco_utils.dataset.numpy_splitters import random_stratified
...
def test_random_stratified_splitter(test_specs, python_dataset ):
    ...
    Xtr, Xtst = random_stratified(X, ratio= test_specs['ratio'], seed= test_specs['seed'])
    ...
    assert( ( Xtr_rated.sum()/(X_rated.sum() ) <= test_specs["ratio"] + test_specs["tolerance"] )
            & (Xtr_rated.sum()/(X_rated.sum() ) >= test_specs["ratio"] - test_specs["tolerance"] )
            )

    assert( ( Xtst_rated.sum()/(X_rated.sum() ) <= (1-test_specs["ratio"]) + test_specs["tolerance"] )
            & (Xtr_rated.sum()/(X_rated.sum() ) >= (1-test_specs["ratio"]) - test_specs["tolerance"] )
            )
    ...
    assert( ( Xtr_rated/X_rated <= test_specs["ratio"] + test_specs["fluctuation"] ).all()
            & ( Xtr_rated/X_rated >= test_specs["ratio"] - test_specs["fluctuation"] ).all()
            )

    assert( ( Xtst_rated/X_rated <= (1-test_specs["ratio"]) + test_specs["fluctuation"] ).all()
            & ( Xtst_rated/X_rated >= (1-test_specs["ratio"]) - test_specs["fluctuation"] ).all()
            )
