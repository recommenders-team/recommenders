...
from reco_utils.evaluation.python_evaluation import (
    PythonRatingEvaluation,
    PythonRankingEvaluation,
)
...
def test_python_rmse(python_data, target_metrics):
    ...
    rating_true, rating_pred = python_data
    evaluator1 = PythonRatingEvaluation(
...
def test_python_ndcg_at_k(python_data, target_metrics):
    ...
    rating_true, rating_pred = python_data
    evaluator1 = PythonRankingEvaluation(
...
def test_python_errors(python_data):
    ...
    with pytest.raises(ValueError):
        PythonRatingEvaluation(
            rating_pred, rating_pred, col_rating="prediction", col_user="not_user"
        )
    ...
    with pytest.raises(ValueError):
        PythonRatingEvaluation(
            rating_pred, rating_pred, col_rating="prediction", col_item="not_item"
        )
    ...
    with pytest.raises(ValueError):
        PythonRatingEvaluation(
            rating_true, rating_pred, col_prediction="not_prediction"
        )
