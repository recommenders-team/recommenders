...
The MovieLens benchmark can be seen at [movielens.ipynb](movielens.ipynb). In this notebook, the MovieLens dataset is split into training / test sets using a stratified splitting method that takes 75% of each user's ratings as training data, and the remaining 25% ratings as test data. For ranking metrics we use `k=10` (top 10 recommended items). The algorithms used in this benchmark are [ALS](../notebooks/00_quick_start/als_movielens.ipynb), [SVD](../notebooks/02_model_collaborative_filtering/surprise_svd_deep_dive.ipynb), [SAR](../notebooks/00_quick_start/sar_movielens.ipynb), [NCF](../notebooks/00_quick_start/ncf_movielens.ipynb), [BPR](../notebooks/02_model_collaborative_filtering/cornac_bpr_deep_dive.ipynb), [BiVAE](../notebooks/02_model_collaborative_filtering/cornac_bivae_deep_dive.ipynb) and [FastAI](../notebooks/00_quick_start/fastai_movielens.ipynb).
