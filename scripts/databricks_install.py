...
# This script installs Recommenders/reco_utils as a library into Databricks
...
PYPI_O16N_LIBS=["azure-cli==2.0.56", "azureml-sdk[databricks]==1.0.8", "pydocumentdb==2.3.3"]


def create_egg(path_to_recommenders_repo_root=os.getcwd(),local_eggname='Recommenders.egg', overwrite=False):
  """
...
def prepare_for_operationalization(cluster_id, api_client, status=None, dbfs_path="dbfs:/FileStore/jars", overwrite=False):
  """
  ...
  DbfsApi(api_client).cp(recursive=False, src=local_jarname, dst=uploadpath, overwrite=overwrite)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--profile", 
      ...
      default="DEFAULT")
  parser.add_argument("--path-to-recommenders",
  ...
  parser.add_argument("--overwrite",
      ...
      default=False)
  parser.add_argument("--prepare-o16n",
      action="store_true",
      help="Whether to install additional libraries for operationalization.",
      default=False)
  parser.add_argument("cluster_id", 
      help="cluster id for the cluster to install on.")
  ...
  myegg = create_egg(args.path_to_recommenders, local_eggname=args.eggname, overwrite=args.overwrite)
  ...
  ## install the library:
  print("Installing the reco_utils module onto databricks cluster {}".format(args.cluster_id))
  ...
  if args.prepare_o16n:
    prepare_for_operationalization(
      cluster_id=args.cluster_id, api_client=my_api_client,
      status=status, dbfs_path=args.dbfs_path, overwrite=args.overwrite)
